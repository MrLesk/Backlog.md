...
async function testDevelopmentMode(verbose: boolean): Promise<TestSuite> {
	...
	await withCleanEnvironment(async () => {
		...
		results.push(await runTest("Development MCP Config Generation", async () => {
			const testEnv = setupDevelopmentEnvironment("dev-config");

			try {
				setupMcpConfiguration(testEnv.dir, "development");

				const { readFileSync } = await import("node:fs");
				const configPath = resolve(testEnv.dir, ".mcp.json");
				const config = JSON.parse(readFileSync(configPath, 'utf8'));

				if (!config.mcpServers || !config.mcpServers["backlog-md"]) {
					throw new Error("Invalid MCP configuration generated");
				}

				const server = config.mcpServers["backlog-md"];
				if (server.command !== "node") {
					throw new Error(`Expected 'node' command, got '${server.command}'`);
				}

				return { configGenerated: true, command: server.command };
			} finally {
				testEnv.cleanup();
			}
		}));

		// Test 4: Wrapper Script Behavior
...
async function testGlobalMode(verbose: boolean): Promise<TestSuite> {
	...
	await withCleanEnvironment(async () => {
		...
		// Test 4: Global MCP Configuration
		results.push(await runTest("Global MCP Config Generation", async () => {
			const testEnv = setupGlobalEnvironment("global-config");

			try {
				setupMcpConfiguration(testEnv.dir, "global");
				...
				const server = config.mcpServers["backlog-md"];
				if (server.command !== "backlog") {
					throw new Error(`Expected 'backlog' command, got '${server.command}'`);
				}

				if (!server.args.includes("--stdio")) {
					throw new Error("Missing --stdio argument in global config");
				}
