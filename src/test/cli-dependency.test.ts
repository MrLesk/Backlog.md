...
// Helper function to run CLI commands using sync spawn to match other tests
function runCLI(args: string[], cwd: string) {
	const result = Bun.spawnSync(["bun", join(process.cwd(), "src", "cli.ts"), ...args], {
		cwd,
		stdout: "pipe",
		stderr: "pipe",
	});
...
describe("CLI Dependency Support", () => {
	...
	test("should create task with single dependency using --dep", async () => {
		...
		const result1 = runCLI(["task", "create", "Base Task"], TEST_DIR);
		expect(result1.exitCode).toBe(0);

		// Create task with dependency
		const result2 = runCLI(["task", "create", "Dependent Task", "--dep", "task-1"], TEST_DIR);
	...
	test("should create task with single dependency using --depends-on", async () => {
		...
		// Create task with dependency
		const result2 = runCLI(["task", "create", "Dependent Task", "--depends-on", "task-1"], TEST_DIR);
		expect(result2.exitCode).toBe(0);
		expect(result2.stdout).toContain("Created task task-2");
	...
	test("should create task with multiple dependencies (comma-separated)", async () => {
		...
		const result1 = runCLI(["task", "create", "Base Task 1"], TEST_DIR);
		expect(result1.exitCode).toBe(0);
		const result2 = runCLI(["task", "create", "Base Task 2"], TEST_DIR);
		expect(result2.exitCode).toBe(0);

		// Create task with multiple dependencies
		const result3 = runCLI(["task", "create", "Dependent Task", "--dep", "task-1,task-2"], TEST_DIR);
	...
	test("should create task with multiple dependencies (multiple flags)", async () => {
		...
		const result3 = runCLI(["task", "create", "Dependent Task", "--dep", "task-1", "--dep", "task-2"], TEST_DIR);
		expect(result3.exitCode).toBe(0);
		expect(result3.stdout).toContain("Created task task-3");
	...
	test("should normalize task IDs in dependencies", async () => {
		...
		const result2 = runCLI(["task", "create", "Dependent Task", "--dep", "1"], TEST_DIR);
		expect(result2.exitCode).toBe(0);
		expect(result2.stdout).toContain("Created task task-2");
	...
	test("should fail when dependency task does not exist", async () => {
		...
		const result = runCLI(["task", "create", "Dependent Task", "--dep", "task-999"], TEST_DIR);
	...
	test("should edit task to add dependencies", async () => {
		...
		const result2 = runCLI(["task", "create", "Base Task 2"], TEST_DIR);
		expect(result2.exitCode).toBe(0);
		const result3 = runCLI(["task", "create", "Task to Edit"], TEST_DIR);
		expect(result3.exitCode).toBe(0);

		// Edit task to add dependencies
		const result4 = runCLI(["task", "edit", "task-3", "--dep", "task-1,task-2"], TEST_DIR);
	...
	test("should edit task to update dependencies", async () => {
		...
		const result2 = runCLI(["task", "create", "Base Task 2"], TEST_DIR);
		expect(result2.exitCode).toBe(0);
		const result3 = runCLI(["task", "create", "Base Task 3"], TEST_DIR);
		expect(result3.exitCode).toBe(0);

		// Create task with initial dependency
		const result4 = runCLI(["task", "create", "Task with Dependency", "--dep", "task-1"], TEST_DIR);
		expect(result4.exitCode).toBe(0);

		// Edit task to change dependencies
		const result5 = runCLI(["task", "edit", "task-4", "--dep", "task-2,task-3"], TEST_DIR);
	...
	test("should handle dependencies on draft tasks", async () => {
		...
		const result1 = runCLI(["task", "create", "Draft Task", "--draft"], TEST_DIR);
		expect(result1.exitCode).toBe(0);
		expect(result1.stdout).toContain("Created draft task-1");

		// Create task that depends on draft
		const result2 = runCLI(["task", "create", "Task depending on draft", "--dep", "task-1"], TEST_DIR);
		expect(result2.exitCode).toBe(0);
	...
	test("should display dependencies in plain text view", async () => {
		...
		const result3 = runCLI(["task", "task-2", "--plain"], TEST_DIR);
		expect(result3.exitCode).toBe(0);
