	...
	describe("board and sequence management tools", () => {
		...
		describe("sequence management", () => {
			...
			it("should create sequence with no tasks", async () => {
				...
				const sequenceData = JSON.parse((result.content[0]?.text || "{}") as string);
			...
			it("should include completed tasks when requested", async () => {
				...
				const sequenceData1 = JSON.parse((result1.content[0]?.text || "{}") as string);
				...
				const sequenceData2 = JSON.parse((result2.content[0]?.text || "{}") as string);
			...
			it("should plan sequence execution with phases", async () => {
				...
				const planData = JSON.parse((result.content[0]?.text || "{}") as string);
			...
			it("should plan sequence for specific task IDs", async () => {
				...
				const totalTasksInPlan = planData.summary.totalTasksInPlan + planData.unsequenced.length;
			...
			it("should handle sequence operations with edge cases", async () => {
				...
				const sequenceData = JSON.parse((result2.content[0]?.text || "{}") as string);
		...
		describe("board and sequence integration", () => {
			it("should work together to provide comprehensive project view", async () => {
				...
				const sequenceData = JSON.parse((sequenceResult.content[0]?.text || "{}") as string);
