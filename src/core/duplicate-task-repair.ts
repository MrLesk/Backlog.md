...
import { extractAnyPrefix, generateNextId, idForFilename } from "../utils/prefix-config.ts";
...
async function findReferenceReviews(core: Core, groupIds: string[]): Promise<DuplicateReferenceReview[]> {
	...
	for (const file of files.sort((left, right) => left.localeCompare(right))) {
		...
		const content = await Bun.file(join(core.filesystem.backlogDir, file))
			.text()
			.catch(() => "");
...
export async function previewDuplicateTaskIdRepair(
	...
	let nextId = groups.length > 0 ? await core.generateNextId(EntityType.Task) : "";
	...
	for (const group of groups) {
		...
		for (const task of tasks.slice(1)) {
			...
			nextId = generateNextId([...existingIds, ...plannedIds], config?.prefixes?.task ?? "task", config?.zeroPaddedIds);
		}
	}
	...
	const references = await findReferenceReviews(
		core,
	...
	return {
		...
		repairable: groups.length > 0 && blockedReasons.length === 0,
		fingerprint,
	};
}
