...
export function calculateQualityMetrics(tasks: readonly Task[]): QualityMetrics {
	...
	const tasksWithAcceptanceCriteria = createTaskCount(
		tasks.filter((t) => t.acceptanceCriteria && t.acceptanceCriteria.length > 0).length,
	);
	...
	const averageTaskComplexity =
		totalTasks > 0
			? tasks.reduce((sum, task) => {
					...
					if (task.acceptanceCriteria && task.acceptanceCriteria.length > 2) complexity += 1;
...
export function calculateTrends(tasks: readonly Task[], timeframe: AnalysisTimeframe): ProjectTrends {
	...
	for (let i = 0; i < periods; i++) {
		...
		trendData.push({
			...
			change: i > 0 ? periodTasks.length - trendData[i - 1]?.value : 0,
			changeDirection:
				i > 0
					? periodTasks.length > trendData[i - 1]?.value
						? "up"
						: periodTasks.length < trendData[i - 1]?.value
							? "down"
							: "stable"
