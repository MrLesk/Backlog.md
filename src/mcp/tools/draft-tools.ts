...
/**
 * Draft promote tool schema
 */
const draftPromoteSchema: JsonSchema = {
	type: "object",
	properties: {
		id: {
			type: "string",
 ...
 */
const createDraftPromoteTool = (handlers: DraftToolHandlers, server: McpServer): McpToolHandler =>
	createAsyncValidatedTool(
		{
			name: "draft_promote",
			description: "Promote a draft to a task",
			inputSchema: draftPromoteSchema,
		},
		draftPromoteSchema,
		async (input, context) => {
...
export function registerDraftTools(server: McpServer): void {
	...
	server.addTool(createDraftPromoteTool(handlers, server));
