...
<!-- SECTION:PLAN:BEGIN -->
1. src/mcp/utils/schema-generators.ts: add generateProjectFieldSchema(config) mirroring generatePriorityFieldSchema, using enum + enumCaseInsensitive; return undefined/omit when config.projects is empty. Wire into task_create and task_edit schemas only (list/search filter schema deferred to BACK-637.4).
...
4. src/types/task-edit-args.ts and src/utils/task-edit-builder.ts already have project wired from BACK-637.2 (shared with CLI) -- verify only.
