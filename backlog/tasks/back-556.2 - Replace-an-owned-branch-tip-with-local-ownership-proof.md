...
Ownership must be exact-SHA based and repository-local, and must fail closed whenever evidence is missing, stale, malformed, or ambiguous. It must not add notes or side refs that change `git rev-list --all` results, and it must not introduce any network operation. The exact evidence format, the rule that matches a candidate `HEAD` against it, and the definition of stale evidence must be specified in one place and covered by tests rather than left to inference.

The existing selected-path robustness stays intact: temporary-index isolation, owned-index reconciliation, retries, signing, legacy and modern hook runners, and atomic expected-old-SHA branch updates. Replacement must additionally match the observable Git rewrite lifecycle, including `post-rewrite` with the old and new commit IDs and the `amend` argument.
...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 The Git layer can replace the current branch tip with a commit that keeps the original parent set, author identity, author date, and signing behavior of the commit it replaces, and refreshes committer metadata and commit ID.
...
- [ ] #7 Ownership tracking adds no notes or side refs, so git rev-list --all output is unchanged by the feature.
...
- [ ] #9 A repository that cannot record ownership evidence, for example one with core.logAllRefUpdates disabled, always reports the tip as not owned.
...
- [ ] #12 The replacement path runs pre-commit, prepare-commit-msg, commit-msg, and post-commit consistently with Git amend semantics and invokes exactly one post-rewrite amend carrying the old and new commit IDs, while the new-commit path invokes no post-rewrite.
...
- [ ] #15 Git-level tests cover repeated replacement sequences, root commits, manual and publication boundaries, hooks and signing, linked worktrees and branch switches, and concurrent branch movement.
<!-- AC:END -->
